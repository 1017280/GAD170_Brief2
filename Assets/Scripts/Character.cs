...
public class Character : MonoBehaviour
{
    ...
    {
        mojoRemaining = Mathf.Max(mojoRemaining - amount, 0);
        BattleLog.Log(charName.GetFullCharacterName() + " took " + amount.ToString() + " damage, " + mojoRemaining + " Mojo remains!", new Color(0.8f, 0.4f, 0.8f, 1.0f));

        if (mojoRemaining == 0)
        {
            BattleLog.Log(charName.GetFullCharacterName() + " died!", Color.red);
